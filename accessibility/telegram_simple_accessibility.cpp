// telegram_simple_accessibility.cpp
...
//   * A diagnostic log is written to <Desktop>/tg_simple_log.txt on each
...
#include "ui/accessibility/telegram_simple_accessibility.h"
#include "ui/accessibility/telegram_simple_keyboard.h"
#include "ui/accessibility/telegram_simple_names.h"
#include "ui/accessibility/telegram_simple_live.h"
...
// =====================================================================
// Diagnostic logging — writes to <Desktop>/tg_simple_log.txt
...
void OpenLog() {
    ...
    const QString path = dir + QStringLiteral("/tg_simple_log.txt");
...
void Install() {
    ...
    Log(QStringLiteral("[TgSimpleA11y] Install() called"));
    ...
    Ui::ScreenReaderModeActiveValue(
        ...
        // names (confirmed in tg_simple_diag.txt / tg_simple_log.txt).
        //
        ...
        const auto nudge = [] {
            QWidget *root = TgSimpleA11y::detail::FindMainWindow();
            if (!root) return;
            QWidget *list = TgSimpleA11y::detail::FindByType(
    ...
    const auto applyNames = [] {
        if (QWidget *root = TgSimpleA11y::detail::FindMainWindow()) {
            ApplyNames(root);
            Log(QStringLiteral("[TgSimpleA11y] ApplyNames() on main window"));
        }
    };
    QTimer::singleShot(3000, qApp, applyNames);
    QTimer::singleShot(8000, qApp, applyNames);

    qDebug() << "[TgSimpleA11y] Screen-reader accessibility layer installed.";
}
