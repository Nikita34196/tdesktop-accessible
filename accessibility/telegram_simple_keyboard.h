...
#include "ui/accessibility/telegram_simple_speech.h"
...
// our tg_simple_diag.txt those events DO fire (focusChild returns the
                    ...
                    if (auto *iface = QAccessible::queryAccessibleInterface(alive.data())) {
                        if (auto *child = detail::FocusedListChild(iface)) {
                            ...
                            if (!detail::IsUselessListItemName(raw)) {
                                nvda::SpeakForced(detail::CompactChatLabel(raw));
                            }
                        }
                    }
                });
                            ...
                            if (auto *child = detail::FocusedListChild(iface)) {
                                ...
                                if (type == QLatin1String("HistoryInner")) {
                                    if (!detail::IsUselessListItemName(name)) {
                                        nvda::SpeakForced(detail::CompactPhrase(name));
                                    }
                                }
                            } else {
                                summary += QStringLiteral(
...
private:
    ...
    void cyclePanels(bool reverse) {
        const auto panels = discoverPanels();
        if (panels.isEmpty()) {
            qDebug() << "[TgSimpleA11y] F6: no panels discovered yet";
            LogLine(QStringLiteral(
    ...
    void focusAndAnnounce(QWidget *w, const QString &name) {
        ...
        if (detail::DynamicTypeName(w) == QLatin1String(
            ...
            QTimer::singleShot(0, [alive] {
                ...
                LogLine(QStringLiteral(
                    ...
                    .arg(int(child->role())));
                if (!detail::IsUselessListItemName(name)) {
                    nvda::SpeakForced(detail::CompactChatLabel(name));
                }
            });
        ...
        qDebug().noquote() << "[TgSimpleA11y]" << line;
        LogLine(line);
    }
};
...
inline void InstallKeyboardNavigation() {
    ...
    if (installed) return;
    if (!qApp) {
        qWarning() << "[TgSimpleA11y] qApp not ready; "
    ...
    installed = true;
    qDebug() << "[TgSimpleA11y] F6 keyboard navigation installed";
}
